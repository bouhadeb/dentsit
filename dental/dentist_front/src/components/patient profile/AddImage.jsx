...
const AddImage = ({ id }) => {
  ...
  const handleImageUpload = async (event) => {
    ...
    // Add all existing patient data to the formData
    ...
    try {
      ...
      // Update the patient data with the new images
      setPatient(response.data);
    } catch (err) {
      console.error(err);
      setUploadError("Échec de la mise à jour des images. Veuillez réessayer.");
    } finally {
      setUploading(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-gray-600">Chargement des images du patient...</p>
        </div>
      </div>
    );
  }
        ...
        <>
          ...
          {patient.images && patient.images.length > 0 ? (
                ...
                {patient.images.map((image, index) => (
                  <div key={index} className="relative">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/50 to-transparent p-4">
                      <p className="text-white text-sm">Image {index + 1} sur {patient.images.length}</p>
                    </div>
                  </div>
                ))}
              </Carousel>
            ...
            <div className="flex flex-col items-center gap-4 p-8 bg-gray-50 rounded-lg">
              <p className="text-gray-600 text-lg">Aucune image disponible pour ce patient</p>
              <label className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors cursor-pointer">
                {uploading ? 'Téléchargement en cours...' : 'Ajouter de nouvelles images'}
                <input
                  type="file"
                  accept="image/*"
                  multiple
